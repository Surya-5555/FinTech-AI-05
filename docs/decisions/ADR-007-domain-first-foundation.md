...
## Rationale
1. **Framework Agnosticism:** The `libs/domain` package is pure TypeScript. It contains absolutely no dependencies on NestJS (`@nestjs/core`), Prisma (`@prisma/client`), or Express. This guarantees that the business logic can be unit-tested instantly without a database or HTTP server.
