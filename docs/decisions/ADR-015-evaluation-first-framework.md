...
2. **Baselines**:
   - `Baseline0`: No Recovery (baseline test).
...
3. **Hidden Ground Truth**: Each test case contains a `groundTruth` object representing real-world behavior (e.g. if the user will pay the link, or if it will permanently fail). The SUT cannot see this label during planning, but the Runner uses it to evaluate outcomes.
...
- External dependencies (LLMs, Razorpay) are swapped with test adapters or disabled during `make evaluate`.
